...
public class StringGui extends JFrame {
  ...
  public StringGui(final JVstHost2 vst) {
    ...
    for (int i = 0; i < 36; i++) {
      ...
      final int note = i;
      int test = i % 12;
      if (test == 0 || test == 2 || test == 4 || test == 5 || test == 7 || test == 9 || test == 11) {
      ...
      key.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent event) {
          try {
            ...
            smNoteOn.setMessage(NOTE_ON, 0, 48 + note, 96);
...
//      }
      for (int i = 0; i < 127; i++) {
        progNames[i] = "Program " + i;
      }

      vst.setProgram(0);

      setProgramViaMidi(vst, 0);
  ...
  private void setProgramViaMidi(JVstHost2 vst, int index) {
//    ShortMessage test2 = new ShortMessage();
//    //try {
//      //test2.setMessage(CONTROL_CHANGE, 97, 0);
//      //test2.setMessage(PROGRAM_CHANGE,0, 1, 2);
//      ShortMessage smNoteOn = new ShortMessage();
//      vst.queueMidiMessage(smNoteOn);
