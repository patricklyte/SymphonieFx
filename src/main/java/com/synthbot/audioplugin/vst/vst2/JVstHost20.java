...
public class JVstHost20 extends JVstHost2 {
  ...
  @Override
  public synchronized void openEditor(String frameTitle) {
    if (frameTitle == null) {
      logger.error("frameTitle may not be null.");
      frameTitle="Vst Editor";
    }
    assertNativeComponentIsLoaded();
    assertHasNativeEditor();
    if (!isEditorOpen()) {
      final JVstHost2 thisJVstHost = this;
      String finalFrameTitle = frameTitle;
      editorThread = new Thread(new Runnable() {
        public void run() {
          openEditor(finalFrameTitle, vstPluginPtr); // this method blocks while the native window is open
          editorThread = null;
  ...
  protected void assertHasNativeEditor() {
    if (!hasNativeEditor) {
      logger.error("This plugin has no native editor. Do not try to manipulate it.");
    }
  }
  ...
  public synchronized void setProgram(int index) {
    ...
    if (index < 0 || index >= numPrograms) {
      logger.error("The program index must be in [0, " + (numPrograms-1) + "): " + Integer.toString(index));
      return;
    }
  ...
  public synchronized void setBankChunk(byte[] chunkData) {
    assertNativeComponentIsLoaded();
    if (chunkData == null) {
      logger.error("Chunk data cannot be null.");
      return;
    }
  ...
  public void setTimeSignature(int numerator, int denominator) {
    if (numerator <= 0) {
      logger.error("The number of note values per measure must be positive: " + Integer.toString(numerator));
      return;
    }
    if (denominator <= 0) {
      logger.error("The note value per beat must be positive: " + Integer.toString(denominator));
      return;
    }
