...
public abstract class JVstHost2 {
    ...
    public static JVstHost2 newInstance(File file, float sampleRate, int blockSize) throws FileNotFoundException, JVstLoadException {
        logger.debug("VstHost2 newInstance");
        JVstHost2 vst = newInstance(file);
        vst.setSampleRate(sampleRate);
        vst.setBlockSize(blockSize);
        logger.debug("VstHost2 turnOn()");
        vst.turnOn();
        logger.debug("VstHost2 turnOn() success");
        return vst;
    }
    ...
    public static JVstHost2 newInstance(File file) throws FileNotFoundException, JVstLoadException {
        ...
        //long pluginPtr =  asyncLoadPlugin(file);
  ...
  static long pluginPtr = 0;
//  static long asyncLoadPlugin(File file) {
//      loaded = false;
//      pluginPtr = 0;
//      Thread loader = new Thread(new Runnable() {
//            public void run() {
//                try {
//                    pluginPtr = loadPlugin(file.toString());
//                    loaded = true;
...
//        loader.setName("Java Plugin Loader " + file.getPath());
//        loader.start();
//        long start = System.currentTimeMillis();
//        long time = 0;
//        while (!loaded && (time < 30000L)) {
//            try {
//                Thread.sleep(500);
...
//        }
//        if(!loaded) {
//            logger.debug("Loading of VST Plugin failed " + file.getPath() ) ;
//          loader.interrupt();
//          loader = null;
//        }
//        return pluginPtr;
//    }
    ...
    public static boolean jvsthost2Available = false;
    ...
    private static void loaddll32bit() {
        try {
            // load jvsthost2_32.dll or jvsthost2_64.dll depending on the the bit-depth of the JVM
            String test = System.mapLibraryName("jvsthost2x_" + System.getProperty("sun.arch.data.model"));
            System.loadLibrary("jvsthost2" + System.getProperty("sun.arch.data.model"));
            jvsthost2Available = true;
            logger.debug("VST 32 Bit enable");
        } catch (Throwable t) {
            System.out.println("failed to load jvsthost2" + System.getProperty("sun.arch.data.model"));
            //c:\Windows\SysWOW64\
        }
    }

    private static void loadDll64() {
//        String filename = "z:/Java/Projects/jvsthost-64bit/res/jvsthost2_64x.dll";
//        File test1 = new File(filename);
//        if(!test1.exists()) {
//            filename = "jvsthost2_64.dll";
//        }
//        test1 = new File(filename);
//        if(!test1.exists()) {
//            filename = "c:\\Windows\\system32\\jvsthost2_64.dll";;
//        }
//        test1 = new File(filename);
//        if(!test1.exists()) return;
        try {
            System.loadLibrary("jvsthost2_64");
            //System.load(filename);
            jvsthost2Available = true;
            logger.debug("VST 64 Bit enable");
        } catch (Throwable t) {
            System.out.println("failed to load " + "c:\\Windows\\system32\\jvsthost2_64.dll");
        }
    }
