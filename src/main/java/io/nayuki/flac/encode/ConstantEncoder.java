...
final class ConstantEncoder extends SubframeEncoder {
	...
	public static SizeEstimate<SubframeEncoder> computeBest(final long[] samples, int shift, int depth) {
	...
	public ConstantEncoder(final long[] samples, final int shift, final int depth) {
		super(shift, depth);
	}
	...
	public void encode(final long[] samples, final BitOutputStream out) throws IOException {
		if (!isConstant(samples))
