...
final class FrameEncoder {
	...
	public static SizeEstimate<FrameEncoder> computeBest(final int sampleOffset, final long[][] samples, final int sampleDepth, final int sampleRate, final SubframeEncoder.SearchOptions opt) {
		final FrameEncoder enc = new FrameEncoder(sampleOffset, samples, sampleDepth, sampleRate);
		...
		if (numChannels != 2) {
			...
			final long[] left  = samples[0];
			final long[] right = samples[1];
			final long[] mid  = new long[samples[0].length];
			final long[] side = new long[mid.length];
			...
			final long mode1Size = leftInfo.sizeEstimate + rightInfo.sizeEstimate;
			final long mode8Size = leftInfo.sizeEstimate + sideInfo.sizeEstimate;
			final long mode9Size = rightInfo.sizeEstimate + sideInfo.sizeEstimate;
			final long mode10Size = midInfo.sizeEstimate + sideInfo.sizeEstimate;
			final long minimum = Math.min(Math.min(mode1Size, mode8Size), Math.min(mode9Size, mode10Size));
