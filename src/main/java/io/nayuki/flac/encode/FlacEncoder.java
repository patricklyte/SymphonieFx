...
public class FlacEncoder {
	...
	public FlacEncoder(final StreamInfo info, final int[][] samples, final int blockSize, final SubframeEncoder.SearchOptions opt, BitOutputStream out) throws IOException {
		info.minBlockSize = blockSize;
		info.maxBlockSize = blockSize;
		info.minFrameSize = 0;
		info.maxFrameSize = 0;
		for (int i = 0, pos = 0; pos < samples[0].length; i++) {
			notifyUI(100.0 * pos / samples[0].length);
			int n = Math.min(samples[0].length - pos, blockSize);
			final long[][] subsamples = getRange(samples, pos, n);
